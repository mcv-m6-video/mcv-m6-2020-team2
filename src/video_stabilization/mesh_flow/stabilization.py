...
from src.video_stabilization.mesh_flow.optimization import real_time_optimize_path
from src.video_stabilization.mesh_flow.mesh_flow import motion_propagate, mesh_warp_frame, generate_vertex_profiles
...
@measure_performance
def read_video(cap, video_percentage):
    """
    ...
    frame_count = int(cap.get(cv2.CAP_PROP_FRAME_COUNT) * video_percentage)
    ...
    bar.close()
    return x_motion_meshes, y_motion_meshes, x_paths, y_paths, HORIZONTAL_BORDER, VERTICAL_BORDER
...
@measure_performance
def generate_stabilized_video(cap, out_path, video_percentage,
                              x_motion_meshes, y_motion_meshes, new_x_motion_meshes, new_y_motion_meshes,
                              HORIZONTAL_BORDER, VERTICAL_BORDER, draw_vectors=False):
    """
    @param: cap is the cv2.VideoCapture object that is
            instantiated with given video
    @param: x_motion_meshes is the motion vectors on
    ...
    out = imageio.get_writer(f'{out_path}/stable.gif', fps=frame_rate)


    for frame_num in tqdm(range(frame_count), desc="Generate gif"):
        ...
        reduced_shape = (int(output.shape[1] / 3), int(output.shape[0] / 3))
        output = cv2.resize(output, reduced_shape, interpolation=cv2.INTER_CUBIC)
        out.append_data(output)
        ...
        if draw_vectors:
            ...
            cv2.imwrite(f'{out_path}/old_motion_vectors/'+str(frame_num)+'.jpg', frame)
            ...
            cv2.imwrite(f'{out_path}/new_motion_vectors/'+str(frame_num)+'.jpg', new_frame)
...
def mesh_flow_main(cap, out_path, video_percentage):
    ...
    x_motion_meshes, y_motion_meshes, x_paths, y_paths, HORIZONTAL_BORDER, VERTICAL_BORDER = read_video(cap, video_percentage)
    ...
    x_motion_meshes, y_motion_meshes, new_x_motion_meshes, new_y_motion_meshes = get_frame_warp(x_motion_meshes, y_motion_meshes,
                                                                                                x_paths, y_paths, sx_paths, sy_paths)

    # apply updated mesh warps & save the result
    generate_stabilized_video(cap, out_path, video_percentage,
                              x_motion_meshes, y_motion_meshes, new_x_motion_meshes, new_y_motion_meshes, HORIZONTAL_BORDER, VERTICAL_BORDER)
